...
for run in runs.keys():
    try:
        ...
        if "b073" not in run:
            linestyle = '--'
        ax.plot([i for i in disk_bulk_composition.keys() if bse_composition[i] != 0], [disk_bulk_composition[i] / bse_composition[i] for i in disk_bulk_composition.keys() if bse_composition[i] != 0], linestyle=linestyle,
                linewidth=2.0, label=to_dir)
...
for run in runs.keys():
    try:
        ...
        print(theia_x_si['Mg'], theia_x_al['Mg'])
        metadata = {
            "temperature": temperature,
            "vmf": vmf,
            "mg/si": theia_x_si['Mg'],
