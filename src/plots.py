...
def collect_data(path, x_header, metadata_rows=9, y_header=None):
    data = {}
    metadata = {}
    ordered_data = {}
...
def collect_metadata(path, x_header, metadata_rows=9):
    data = {}
    metadata = {}
    ordered_data = {}
    iterations = list(sorted([int(i.split("/")[-1].replace(".csv", "")) for i in os.listdir(path)]))
        ...
        with open(p, 'r') as infile:
            ...
            for j in range(0, metadata_rows):
                ...
                try:  # try to convert to float
                    ...
                    metadata[i].update({line[0]: line[1]})
    for i in list(sorted(metadata.keys())):
        ordered_data.update({metadata[i][x_header]: metadata[i]})
    return ordered_data
